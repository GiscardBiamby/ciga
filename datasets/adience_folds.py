...
def make_folders(dict, base_path):
    for split in ["/train/", "/valid/"]:
        for key,folder_name in dict.items():
            path = os.path.join(base_path, folder_name)
            path = path.replace("/train_or_val/")
            os.makedirs(path)
...
def generateFolds(adience_images, processed_path, fold_num):
    ...
    make_folders(age_group_map, age_base_path)
    make_folders(gender_map, gender_base_path)
    make_folders(age_gender_map, age_gender_base_path)
    ...
    for img in adience_images:
        ...
        # print("fold_num: {}, imgfold: {}, split: {}".format(fold_num, img_fold, split))
...
def main():
    ...
    # for img in adience_images:
    #     if get_face_id(img)=="433":
    #         print("{}, {}, {}, {}, {}".format(get_fold_num(img), get_gender(img), get_age(img), get_filename(img),
    #                                   get_raw_path(img)))
    # return
