...
gender, age, gender_age = root + '/gender', root + '/age', root + '/gender_age'
...
male, female, age_path = root + '/gender/male', root + '/gender/female', root + '/age/'
for j in range(5):
    folds = [np.genfromtxt("fold_%d_data.txt" % j, usecols=(0,1,2,3,4), delimiter='\t', dtype=None)]
    folds += [np.genfromtxt("fold_frontal_%d_data.txt" % j, usecols=(0,1,2,3,4), delimiter='\t', dtype=None)]
    ...
    for fold in folds:
        for i in range(1, fold.shape[0]):
            ...
            face_path = root + '/faces/faces/' + path
            aligned_path = root + '/aligned/aligned/' + path
